...
### adding vehicles
        ...
        transform = random.choice(world1.get_map().get_spawn_points())
        transform.location.x = -0.4771
        transform.location.y = 0.1983 - 19.8
        transform.rotation.yaw = 0
        if False:
            npc = world1.try_spawn_actor(bp, transform)
            if npc is not None:
                #print(bp)
                car_num += 1
...
### adding vehicles finish
        ...
        while True:
            ...
            if (location.x-cx)*(location.x-cx)+(location.y-cy)*(location.y-cy) - r*r <40 and car_num <80:
                bp = random.choice(blueprint_library.filter('vehicle'))
                transform = random.choice(world1.get_map().get_spawn_points())
