...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, opt_dict={}):
        ...
        self._last_blocking_vehicle_buffer = []
    ...
    def _destory_blocking_vehicle(self):
        world_decision = self._vehicle.get_world()
        actor_list = world_decision.get_actors()
        vehicle_list = actor_list.filter("*vehicle*")
        blocking_vehicle_list = []
        blocking_thr = 40
        #print(self._last_blocking_vehicle_buffer)
        for target_vehicle in vehicle_list:
            ...
            if target_vehicle_speed > 0.5:
                continue
            blocking_length = 0
            for saved_vehicle in self._last_blocking_vehicle_buffer:
                if target_vehicle.id == saved_vehicle[0]:
                    blocking_length = saved_vehicle[1]
                    break
            blocking_length = blocking_length + 1
            if blocking_length > blocking_thr:
                target_vehicle.destroy()
            else:
                blocking_vehicle_list.append([target_vehicle.id,blocking_length])

        self._last_blocking_vehicle_buffer = blocking_vehicle_list
    ...
    def run_step(self, debug=True):
        ...
        self._destory_blocking_vehicle()
