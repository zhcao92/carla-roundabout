...
#         cz_time = timestamp.elapsed_seconds
#
...
def game_loop(args):
    ...
    try:
        ...
        #collision_sensor = CollisionSensor(ego_vehicle)
        ...
        bp = random.choice(blueprint_library.filter('carlamotors'))
        transform = random.choice(world.get_map().get_spawn_points())
        transform.location.x = -0.4771
        transform.location.y = 0.1983 - 19.8
        ...
        ### adding vehicles finish

        if args.agent == "Roaming":
            agent = RoamingAgent(ego_vehicle)
        else:
        ...
        while True:
            ...
            if (location.x-cx)*(location.x-cx)+(location.y-cy)*(location.y-cy) - r*r < 40 and car_num <600:
                bp = random.choice(blueprint_library.filter('vehicle'))
                transform = random.choice(world.get_map().get_spawn_points())
                ...
                for car in actor_list:
                    ...
                    if d1<d:
                        d = d1
                if d > 10 and ((transform.location.x-cx)*(transform.location.x-cx)+(transform.location.y-cy)*(transform.location.y-cy)-r*r) > 0:
                    npc = world.try_spawn_actor(bp, transform)
                    if npc is not None:
                        car_num += 1
                        actor_list.append(npc)
